...
static int forth_drv_init(void)//内核入口函数 
{

	major = register_chrdev(0,"forth_drv",&forth_drv_fops);//注册驱动程序 ，
	forthdrv_class = class_create(THIS_MODULE,"forth_drv");//生成设备信息存放到/sys目录为mdev自动创建设备节点 此处创建一个类second_drv 放在sys目录下
	forthdrv_class_dev = class_device_create(forthdrv_class,NULL,MKDEV(major,0),NULL,"buttons");//在second_drv类下创建一个/dev/buttons设备
...
static void forth_drv_exit(void)//出口函数
{
